namespace Sys (* System Helper module *)
...
module C =
    ...
    let ($) buf l = BitConverter.GetBytes(  Crc16((buf : byte array).Take(l).ToArray()) )   (*--- CRC ---*)

    let (++>) buf l = buf <++ ((buf: byte array) $ l) <| l <| 2; buf                     (*--- CRC & OUT ----*)
